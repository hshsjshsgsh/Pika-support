...
async def lock(ctx, *, args=None):
    ...
    try:
        ...
        # Create new overwrite that explicitly denies send_messages
        new_overwrite = discord.PermissionOverwrite()
        new_overwrite.send_messages = False
        
        # Preserve current read_messages setting if it exists
        if current_overwrite.read_messages is not None:
            new_overwrite.read_messages = current_overwrite.read_messages
        
        await channel.set_permissions(everyone_role, overwrite=new_overwrite)
        ...
        if mentioned_roles:
            for role in mentioned_roles:
                ...
                # Create new overwrite that explicitly allows send_messages
                new_role_overwrite = discord.PermissionOverwrite()
                new_role_overwrite.send_messages = True
                
                # Preserve current read_messages setting if it exists
                if role_overwrite.read_messages is not None:
                    new_role_overwrite.read_messages = role_overwrite.read_messages
                
                await channel.set_permissions(role, overwrite=new_role_overwrite)
    ...
    try:
        ...
        for target, overwrite in current_overwrites.items():
            ...
            if overwrite.send_messages is not None:
                # Create new overwrite preserving other permissions
                new_overwrite = discord.PermissionOverwrite()
                
                # Copy all permissions except send_messages
                if overwrite.read_messages is not None:
                    new_overwrite.read_messages = overwrite.read_messages
                if overwrite.view_channel is not None:
                    new_overwrite.view_channel = overwrite.view_channel
                if overwrite.manage_messages is not None:
                    new_overwrite.manage_messages = overwrite.manage_messages
                if overwrite.embed_links is not None:
                    new_overwrite.embed_links = overwrite.embed_links
                if overwrite.attach_files is not None:
                    new_overwrite.attach_files = overwrite.attach_files
                if overwrite.read_message_history is not None:
                    new_overwrite.read_message_history = overwrite.read_message_history
                if overwrite.mention_everyone is not None:
                    new_overwrite.mention_everyone = overwrite.mention_everyone
                if overwrite.use_external_emojis is not None:
                    new_overwrite.use_external_emojis = overwrite.use_external_emojis
                if overwrite.add_reactions is not None:
                    new_overwrite.add_reactions = overwrite.add_reactions
                
                # send_messages is left as None (default/inherit)
                
                # If the new overwrite is empty, remove it entirely
