...
async def unlock(ctx):
    ...
    try:
        ...
        # Reset send_messages permission for all targets that have it set
        for target, overwrite in current_overwrites.items():
            ...
            if overwrite.send_messages is not None:
                # Create new overwrite from the existing one
                new_overwrite = discord.PermissionOverwrite.from_pair(*overwrite.pair())
                # Reset send_messages to None (inherit default)
                new_overwrite.send_messages = None
